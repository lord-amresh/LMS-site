...
import {
  ...
  Image as ImageIcon,
  BookOpenText,
  Upload, Video, Plus, Clock, ListOrdered,
  PenLine,
  Star,
  X ,
  ChevronDown,
...
// this is also a helper function too and it normalize lecture/chapter total.
...
const EditPage = () => {
        ...
        const fetchCourse = async () => {
            try {
                ...
                if (data.success || data.course) {
                    ...
                    // Helper to format image URL for the preview box
                    ...
                    setFormData({
                        ...
                        image: { file: null, preview: getImageUrl(course.image || course.imageUrl) },
                        rating: course.rating || 0,
                        pricingType: course.pricingType || "free",
                        price: course.price || { original: "", sale: "" },
                        overview: course.overview || "",
                        totalDuration: course.totalDuration || { hours: "", minutes: "" },
                ...
                toast.error("Server error while fetching course");
            } finally {
                setIsFetching(false);
            }
        };
  ...
  // to calculate total minutes, duration + total lectures count
  ...
  const validateForm = () => {
    ...
    if (!formData.image?.file && !formData.image?.preview) {
      toast.error("Course must have an image");
      return false;
    }
    ...
    return true;
  }; // if filled then ok else it will show a error toast
  ...
  };  //similarly to lectures first validate the input fields if ok create
  // a lecture / a chapter.
  
  // add a chapter , remove the lecture or remove a chapter function.
  ...
  const submitToBackend = async () => {
    ...
    try {
      ...
      // We add the ID to the URL and change the method to PUT
      const res = await fetch(`${API_BASE}/api/course/${id}`, {
        method: "PUT", 
        body: fd,  
      });
  ...
  if (isFetching) {
      return <div className="min-h-screen flex items-center justify-center text-xl font-semibold text-indigo-600">Loading Course Data...</div>;
  }
  ...
  const quillModules = {
    toolbar: [
      ...
      ['link', 'image', 'video'], // Media (This adds your image button!)
             ...
             <div className={addPageStyles.formFullWidth}>
              ...
              <div className="bg-white rounded-lg overflow-hidden border border-gray-200">
                <ReactQuill 
                  ...
                  className="h-64 mb-12" // mb-12 prevents the editor toolbar from overlapping the bottom
            ...
            <div className={addPageStyles.modal}>
              ...
              <div className={addPageStyles.modalContent}>
                <div>
                  ...
                  <input type="text" name="title" value={currentLecture.title} 
                  onChange={handleLectureChange} placeholder="e.g., Introduction to React"
                  className={addPageStyles.input} />
                </div>
